...
function WritingHero() {
  return (
    <section className="writing-hero" aria-labelledby="writing-hero-title">
      <Image
        src="/images/essay-behaviour.png"
        alt="Financial analysis and investment decisions"
        fill
        priority
        sizes="100vw"
      />
      ...
      <div className="writing-hero-content container">
        <p className="eyebrow eyebrow-light">
          <span />From my journal
        </p>
        <h1 id="writing-hero-title">Thoughts on finance, business and the forces shaping society.</h1>
        <p>
          Insights and perspectives from Colin McLean on investing, economics,
          business, behaviour and public policy.
        </p>
...
export function WritingPage({ posts, categories: dynamicCategories }: WritingPageProps) {
  ...
  const popularPosts = hasPosts ? blogList.slice(0, 4) : [];
  ...
  const categoryItems =
    dynamicCategories && dynamicCategories.length > 0
      ? dynamicCategories.map((c) => ({
          name: c.name,
          href: `/writing/${c.slug}`,
          count: posts ? posts.filter((p) => p.category?.toLowerCase() === c.name.toLowerCase()).length : 0,
        }))
      : staticCategories;
            ...
            <div className="writing-main">
              {hasPosts && featured ? (
                ...
                <div style={{ textAlign: "center", padding: "5rem 1rem", color: "var(--color-muted, #71717a)" }}>
                  <h2 style={{ fontSize: "1.5rem", marginBottom: "0.5rem" }}>No blogs published yet</h2>
